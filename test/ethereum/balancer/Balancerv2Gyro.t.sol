...
contract BalancerGyroTest is Test {
    ...
    function test_PortalOut() public {
        address token0 = USDC;
        address token1 = GYD;
        address pool = gyroPool;
        ...
        // Mimic sending Tokens to user for the purpose of this test
        deal(pool, recipient, lpAmount);

        // Check the initial WBNB balance of the user
        assertEq(ERC20(pool).balanceOf(recipient), lpAmount);

        // Approve the amount to spend by the portal
        ERC20(pool).approve(address(gyroPortal), lpAmount);
        ...
        uint256 outputBalance0 =
            ERC20(token0).balanceOf(address(recipient));
        uint256 outputBalance1 =
            ERC20(token1).balanceOf(address(recipient));

        console2.log("outputBalance0", outputBalance0);
        console2.log("outputBalance1", outputBalance1);

        assertTrue(
            outputBalance0 > 0 || outputBalance1 > 0,
            "One of the tokens should be 0 after portalIn"
        );

        assertTrue(
            finalBalance == 0,
            "Balance should be zero after PortalOut"
        );
    }
}
